	...
	for(long long i = 0; i < Nsims; i++) {
		...
		while(!g.IsConnected()) {
			...
			for (int i = 0; i < Nwalkers; i++) {
				...
				std::array<float, 4> ws = {0.25,0.5,0.75,1};
			
				float rand = generateRandom(generator,dist);
				int dir = getDirection(ws,rand);
				updatePosition(xs[i],dir,neighbours);
				updateSites(xs[i],i+1,sites);
				// Now we check that it hasn't crossed the boundary.
				if ((dir == 1 && xs[i] < gridLength) ||
				    (dir == 3 && xs[i] > Nsites - gridLength - 1) ||
				    (dir == 0 && xs[i] % gridLength == gridLength - 1) ||
				    (dir == 2 && xs[i] % gridLength == 0)) {
				    	crossed = true;
				}
